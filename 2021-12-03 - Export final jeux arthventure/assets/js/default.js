...
for (let i = 0; i < imgInformation.length; i++) {
    ...
    //console.log("src--------------> " + imgInformation[i]);
  ...
  for (let i = 0; i < img.length; i++) {
    ...
    if (pers.y + pers.height + img[i].height - pad < img[i].height) {
      ...
      if(localStorage.getItem("coucou")==0){
        ...
        document.querySelector(".masthead").style.cssText=`background:url(${newBackground}); background-position:bottom center;`;
      }
...
*/
function scrollHistoire(direction){
  var scrollPos = document.querySelector(".paragraphe").scrollTop;
...
function genererHistoire(lieu){
  document.getElementById('comte').innerHTML = "";
  document.querySelector("a.btn-circle").style.cssText="transform: translateX(-100vw) rotate(360deg); -webkit-transition: 1s 500ms;"; 
  ...
  //objSuiteHistoire.innerHTML = suiteHistoire;
  //document.querySelector("#comte").innerHTML += suiteHistoire;
  ...
  (function type() {
      ...
      document.getElementById('comte').innerHTML = text;
      ...
      if(n[n.length - 1]=="tempête"){
        findepluie = false;
        var evtPluie = setInterval(rainFall, 0.5); // déclenchement de la pluie sur le mot tempête
      }
      document.getElementById('comte').scrollIntoView({block: "end"}); // descente prompteur
      /* ANIMATIONS */
      ...
      if(suiteHistoire.length==document.querySelector("#comte").innerHTML.length+1){
          console.log("fin de l'histoire");
          document.querySelector("a.btn-circle").style.cssText="transform: translateX(0vw) rotate(360deg); -webkit-transition: 1s 500ms;";
          document.querySelector(".paragraphe").style.overflowY = "visible";
          localStorage.setItem("coucou","0");
          document.getElementById("element").style.removeProperty('bottom');
          document.getElementById("element").style.bottom="150px;";
          document.getElementById("element").style.removeProperty('right');
          document.getElementById("element").style.removeProperty('left');
          findepluie = true;
      }
  }());    
...
function suiteHistoire(idCarte){
  switch(idCarte){
      ...
      case 'carte4':
          ...
          break;
      default:
          alert("Oups l'histoire n'est pas reconnu...");
  }
}
...
window.addEventListener('load', function(){
  localStorage.setItem("coucou", "0");
  var mon_element = document.getElementById('element');

    const elementToMove = mon_element;
    ...
    window.addEventListener("keydown", function(event) {
      ...
      switch (keyName) {
        ...
        case "ArrowDown":
          ...
          elementToMove.style.bottom = (parseInt(elementToMove.style.bottom || 150) - 10)+'px';
          elementToMove.style.transition = "all 100ms ease-in-out";
          console.log(isCollide(sizeElemt('element'),sizeElements('img')));
          break;
        ...
        case "ArrowLeft":
          ...
          elementToMove.style.left = (parseInt(elementToMove.style.left || 0) - 10)+'px';
          elementToMove.style.transition = "all 100ms ease-in-out";
          console.log(isCollide(sizeElemt('element'),sizeElements('img')));
          break;
